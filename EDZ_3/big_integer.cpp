...
bool BigInteger::operator==(const BigInteger& other) const {
  return is_negative_ == other.is_negative_ && arr_.size() == other.arr_.size() && std::equal(arr_.begin(), arr_.end(), other.arr_.begin());
}
...
bool BigInteger::operator<(const BigInteger& other) const {
  ...
  if (arr_.size() == 1 && arr_[0] == 0 && other.arr_.size() == 1 && other.arr_[0] == 0) {
    return false;
  }
